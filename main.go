...
// - sudo go run host-manager/main.go -d site.loc -add
// remove site.loc from /etc/hosts
// - sudo go run host-manager/main.go -d site.loc -rm
func main() {
	d := flag.String("d", "", "domain name for action")
	isAdd := flag.Bool("add", false, "add to hosts")
	isRm := flag.Bool("rm", false, "remove from hosts")
	flag.Parse()
